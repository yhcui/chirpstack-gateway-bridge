...
func getUplinkFrame(gatewayID []byte, rxpk RXPK, FakeRxInfoTime bool) (gw.UplinkFrame, error) {
	...
	// Context
	binary.BigEndian.PutUint64(frame.RxInfo.Context, rxpk.Tmst)
	frame.RxInfo.Context[8] = rxpk.RFCh
...
type Stat struct {
	Time CompactTime `json:"time"` // UTC 'system' time of the gateway, ISO 8601 'expanded' format (e.g 2014-01-12 08:59:28 GMT)
...
type RXPK struct {
	...
	Tmst uint64       `json:"tmst"` // Internal timestamp of "RX finished" event (note: changed to uint64)
