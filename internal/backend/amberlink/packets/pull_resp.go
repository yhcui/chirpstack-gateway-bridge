...
type TXPK struct {
	...
	Tmst2 *uint64 `json:"tmst2,omitempty"` // Send packet on a certain timestamp value (will ignore time)
	...
	Freq2 float64 `json:"freq2,omitempty"` // TX central frequency in MHz (unsigned float, Hz precision)
	...
	DatR2 string  `json:"datr2,omitempty"` // LoRa datarate identifier (eg. SF12BW500) || FSK datarate (unsigned, in bits per second)
...
func GetPullRespPacket(protoVersion uint8, randomToken uint16, frame gw.DownlinkFrame) (PullRespPacket, error) {
	...
	if len(frame.TxInfo.Context) >= 9 {
		packet.Payload.TXPK.RFCh = frame.TxInfo.Context[8]
	}
	...
	case gw.DownlinkTiming_DELAY:
		...
		if len(frame.TxInfo.Context) < 8 {
			return packet, fmt.Errorf("context must contain at least 8 bytes, got: %d", len(frame.TxInfo.Context))
		}
		timestamp := binary.BigEndian.Uint64(frame.TxInfo.Context[0:8])
		timestamp += uint64(delay / time.Microsecond)
		packet.Payload.TXPK.Tmst = &timestamp
		packet.Payload.TXPK.Freq2 = 2480
		packet.Payload.TXPK.DatR2 = "SF9BW800"

		timestamp2 := timestamp + 1000000
		packet.Payload.TXPK.Tmst2 = &timestamp2
